...
Let's simulate some gene expression data...
...
for(i in 1:10){ 
  xpr[inx[i],] <- a[i] * rep(mu_lin, each = ni) + rnorm(n)
  xpr[inx[i+10],] <- a[i] * rep(mu_sq2, each = ni) + rnorm(n) 
} 
for(i in 21:100) xpr[inx[i],] <- 3 + rnorm(n)
```
...
Now we can plot the data for an exemplary gene and add the true mean gene expression values (black line): 
```{r, fig.align="center"}
plot(dose, xpr[54,], col = as.factor(dose), lwd = 2, ylab = "expression") 
lines(sort(unique(dose)), mu_lin * a[which(inx == 54)], lty = 1, col = 1) 
```
 
Alternatively, we can plot dose levels (on ordinal scale) against expression values: 
```{r, fig.align="center"}
plot(1:length(sort(unique(dose))), ylim = range(xpr[54,]), pch = "", ylab = "expression", 
     xlab = "levels", xaxt="n")
axis(1, at = 1:length(sort(unique(dose))) ) 
points(as.factor(dose), xpr[54,], col=as.factor(dose), lwd = 2) 
lines(1:length(sort(unique(dose))), mu_lin * a[which(inx == 54)], lty = 1)
``` 

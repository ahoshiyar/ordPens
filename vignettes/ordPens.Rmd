...
such that smooth terms in the GAM formula can be used. In addition, generic functions for prediction and plotting are provided; and existings `mgcv` tools for further statistical inference can be used this way, which enables a very flexible analysis.
...
#### ICF core set for chronic widepsread Pain  
...
The null distribution of RLRT is obtained by simulation (one million values by default).

As an illustrative example let's consider the `dopamine` dataset from package `IsoGene` (Lin et al. 2014). 

As there are dependencies between `IsoGene` and the Bioconductor package `Biobase`, we firstly need to follow the instructions from http://bioconductor.org for installation:  
```{r,warning = FALSE, message = FALSE} 
if (!requireNamespace("BiocManager", quietly = TRUE)) 
  install.packages("BiocManager") 
BiocManager::install(c("Biobase","IsoGene")) 
require(IsoGene)
require(Biobase)  
```

The dataset contains 26 samples of dose-response microarray data of 1000 genes at six doses.

Let's assume that `xpr` contains the gene expression data with genes in the rows and samples in the columns:
```{r}
data(dopamine)
...
After some small modification, `dose` contains the corresponding doses being as follows: 
```{r}
dose <- unlist(pData(dopamine))
table(dose)
```

Now we can plot the data for an exemplary gene and add the mean gene expression values (black line): 
```{r, fig.align="center"}
plot(dose, xpr[83,], col = as.factor(dose), lwd = 2, ylab = "expression") 

xpr.m <- tapply(as.numeric(xpr[83,])[order(dose)], as.factor(dose[order(dose)]), mean)
lines(sort(unique(dose)), xpr.m, lty = 1, col = 1)
```
 
Alternatively, we can plot dose levels (on ordinal scale) against expression values: 
```{r, fig.align="center"}
plot(1:length(sort(unique(dose))),ylim = range(xpr[83,]), pch = "", ylab = "expression", 
     xlab = "levels", xaxt="n")
axis(1, at = 1:length(sort(unique(dose))) )

points(as.factor(dose), xpr[83,], col=as.factor(dose), lwd = 2)
lines(1:length(sort(unique(dose))), xpr.m, lty = 1, col = 1)
``` 
...
Finally, we can compare the empirical cumulative distribution of the smallest p-values ($\leq 0.05$) produced from each of the tests (ANOVA, RLRT, t-test):
```{r, fig.align="center" } 
...
* Lin, D., S. Pramana, T. Verbeke and Z. Shkedy (2014). IsoGene: order-restricted inference for microarray experiments, R package version 1.0-23.

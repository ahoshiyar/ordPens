...
Let’s start with the most basic example with univariate penalty, i.e. `lambda` being a scalar. We extract 5 principal components, specified by the argument `p`.
```{r} 
ehd_pca1 <- ordPCA(H, p = 5, lambda = 1, maxit = 100, crit = 1e-7,
...
We can visualize the estimates, e.g., for Variable *e2*, by selecting the corresponding list entry:
```{r, fig.align="center"}
...
We can apply the function for different values of the penalty parameter. As we specify `lambda` as a vector, the output will result in a list of multivariate matrices.
```{r}
ehd_pca2 <- ordPCA(H, p = 5, lambda = c(0, 1, 10), maxit = 100, crit = 1e-7,
                   Qstart = NULL, Ks = apply(H, 2, max), constr = rep(TRUE, ncol(H)),
                   CV = FALSE)
```
...
`ehd_pca2$qs`: List of matrices (of dimension `Ks x length(lambda)`) with entries corresponding to the variables, e.g. for the second variable (*e2*):  
```{r}
ehd_pca2$qs[[2]]
```
...
```{r, fig.align="center"}
plot(ehd_pca2$qs[[2]][,1], type = "b", xlab = "category", ylab = "quantification", col = 1, 
     ylim = range(c(ehd_pca2$qs[[2]][,1]), ehd_pca2$qs[[2]][,2], ehd_pca2$qs[[2]][,3]), 
     main = colnames(H)[2], bty = "n")
lines(ehd_pca2$qs[[2]][,2], type = "b", col = 2, lty = 2, pch = 2, lwd=2)
lines(ehd_pca2$qs[[2]][,3], type = "b", col = 3, lty = 3, pch = 3, lwd=2)
```
...
for(j in c(1, 9, 12, 13, 15, 19)){ 
  plot(ehd_pca2$qs[[j]][,1], type = "b", main = colnames(H)[j], xlab = "category", 
       ylab = "quantification", lwd = 2, bty = "n") 
  lines(ehd_pca2$qs[[j]][,2], type = "b", col = 2, lty = 2, pch = 2, lwd = 2)
  lines(ehd_pca2$qs[[j]][,3], type = "b", col = 3, lty = 3, pch = 3, lwd = 2)
} 
```
...
We can also compare different amounts of penalization by means of variance acounted for (VAF) for performance evaluation by setting `CV = TRUE` and specifying the number of folds `k`. Use `CVfit = TRUE` to perform both k-fold cross validation and estimation (which, however, can be time-consuming if lambda has many elements). 
```{r}
ehd_pca3 <- ordPCA(H, p = 5, lambda = c(0.1, 1, 10), maxit = 100, crit = 1e-7,
                   Qstart = NULL, Ks = apply(H, 2, max), constr = rep(TRUE, ncol(H)),
                   CV = TRUE, k = 5, CVfit = TRUE)
...
The optimal $\lambda$ can then be determined by the value maximizing the cross-validated VAF on the validation set:
```{r test, figures-side, fig.show="hold", fig.width = 5, fig.height = 5, out.width="45%",out.height="50%"}  
plot(log10(lambda), ehd_pca4$VAFtrain, type = "l",
...
plot(log10(lambda), ehd_pca4$VAFtest, type = "l",
     xlab = expression(log[10](lambda)), ylab = "proportion of variance explained",
     main = "validation data", cex.axis = 1.2, cex.lab = 1.2, cex.main = 1.4)
abline(v = log10(lambda)[which.max(ehd_pca4$VAFtest)])
```
...
icf_pca1 <- ordPCA(H, p = 2, lambda = c(0.1, 1 ,10), maxit = 100, crit = 1e-7, Qstart = NULL, 
                   Ks = c(rep(5,50), rep(9,16), 5), 
                   constr = c(rep(TRUE,50), rep(FALSE,16), TRUE), 
                   CV = FALSE, k = 5, CVfit = FALSE) 
 
icf_pca1C <- ordPCA(H, p = 2, lambda = c(0.1, 1, 10), maxit = 100, crit = 1e-7, Qstart = NULL, 
                    Ks = c(rep(5,50), rep(9,16), 5), constr = rep(TRUE, ncol(H)), 
                    CV = FALSE, k = 5, CVfit = FALSE) 

plot(icf_pca1$qs[[51]][,1], type = "b", xlab = "category", ylab = "quantification", col = 1, 
     ylim = range(c(icf_pca1$qs[[51]][,1], icf_pca1$qs[[51]][,2], icf_pca1$qs[[51]][,3])), 
     main = xnames[51], bty = "n", xaxt = "n") 
lines(icf_pca1$qs[[51]][,2], type = "b", col = 2, lty = 2, pch = 2, lwd=2)
lines(icf_pca1$qs[[51]][,3], type = "b", col = 3, lty = 3, pch = 3, lwd=2)
axis(1, at = 1:length(icf_pca1$qs[[51]][,1]), labels = -4:4)   

plot(icf_pca1C$qs[[51]][,1], type = "b", xlab = "category", ylab = "quantification", col = 1, 
     ylim = range(c(icf_pca1C$qs[[55]][,1], icf_pca1C$qs[[51]][,2], icf_pca1C$qs[[51]][,3])), 
     main = xnames[51], bty = "n", xaxt = "n")  
lines(icf_pca1C$qs[[51]][,2], type = "b", col = 2, lty = 2, pch = 2, lwd=2)
lines(icf_pca1C$qs[[51]][,3], type = "b", col = 3, lty = 3, pch = 3, lwd=2)
...
par(mar = c(4.1, 4.1, 3.1, 1.1))
for (j in wx){
plot(icf_pca1$qs[[j]][,1], type = "b", main = xmain[j], xlab = "category", 
     ylim = range(c(icf_pca1$qs[[j]][,1], icf_pca1$qs[[j]][,2], icf_pca1$qs[[j]][,3])),
     ylab = "quantification", bty = "n", xaxt = "n", cex.main= 1)  
lines(icf_pca1$qs[[j]][,2], type = "b", col = 2, lty = 2, pch = 2, lwd=2)
lines(icf_pca1$qs[[j]][,3], type = "b", col = 3, lty = 3, pch = 3, lwd=2)
